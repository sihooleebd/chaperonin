  ...
  return (
    ...
    >
      ...
      {mod.inputs.length > 0 && (
        <div className="chap-labels chap-labels--top">
          {mod.inputs.map((inp) => (
            <span
              key={inp.id}
              className="chap-label"
              style={{ color: TYPE_COLORS[inp.type] || '#6b7280' }}
            >
              {inp.id}
            </span>
          ))}
      ...
      <div className="chap-header">
        <span className="chap-name" style={{ color: cat.color }}>
          {mod.label}
        </span>
      ...
      <div className="chap-body">
        {status === 'idle' && (
          <div className="chap-meta">
            ...
            <span className="chap-badge" style={{ color: ret.color }}>
              {ret.label}
            </span>
          </div>
        )}
        ...
        {status === 'running' && progress && (
          <div className="chap-progress-wrap">
            ...
            <span className="chap-progress-label">
        ...
        <div className="chap-labels chap-labels--bottom">
          {mod.outputs.map((out) => (
            <span
              key={out.id}
              className="chap-label"
              style={{ color: TYPE_COLORS[out.type] || '#6b7280' }}
            >
              {out.id}
            </span>
          ))}
